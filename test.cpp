...
int main(void)
{
    ...
    Command commands[2];
    commands[0]._fd_in=0; commands[0]._flag_pipe_in=0; commands[0]._fd_out=1; commands[0]._flag_pipe_out=1; commands[0]._commands=ls_cmd;
    commands[1]._fd_in=0; commands[1]._flag_pipe_in=1; commands[1]._fd_out=1; commands[1]._flag_pipe_out=0; commands[1]._commands=grep_cmd;
    int commands_num = sizeof(commands) / sizeof(commands[0]);

    /* Allow valgrind to check memory */
    Command *pcommands = new Command[2];
